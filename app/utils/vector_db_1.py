# -*- coding: utf-8 -*-
"""
Vector DB 1 - FinSolve Internal Chatbot

Automatically generated by Colab.
Original file is located at
    https://colab.research.google.com/drive/1cxidy7ZlCtuJrIjS7vm6WFYceGccqBuz

Vector Database Implementation for FinSolve Internal Chatbot
"""
...
if len(main_splits) > 0:
    ...
    if executive_summary_content:
        ...
        metadatas_new.append(
            {
                "source": "quarterly_financial_report.md",
                "department": "FinSolve Technologies Inc.",
        ...
        metadatas_new.append(
            {
                "source": "quarterly_financial_report.md",
                "department": "FinSolve Technologies Inc.",
                "section": "Q1 (includes placeholder for Q2-Q4)",
            }
        )
...
if len(main_splits) > 2:
    ...
    if len(annual_parts) > 0:
        ...
        if len(annual_summary_segment) > 1 and annual_summary_segment[1].strip():
            temp_chunks.append(
                "## 2024 Annual Summary\n" + annual_summary_segment[1].strip()
            )  # Include header for clarity
            ...
            annual_summary_segment[0].strip()
            and "## 2024 Annual Summary" in annual_summary_segment[0]
        ):
    ...
    if len(recs_parts) > 0:
        ...
        if len(recommendations_segment) > 1 and recommendations_segment[1].strip():
            ...
            recommendations_segment[0].strip()
            and "## Recommendations for 2025" in recommendations_segment[0]
        ):
...
print(
    client.get_collection(collection_name).get(ids=["doc_chunk_3"])["documents"][0][
        :150
    ]
    + "..."
)
