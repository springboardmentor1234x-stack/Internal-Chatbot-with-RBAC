        ...
        if submit:
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    st.success(
                        f"Welcome, {data['user']['username']}! Role: {data['user']['role']}"
                    )
                    st.rerun()
                else:
                ...
                st.error(
                    "❌ Cannot connect to the backend. Please ensure FastAPI is running on http://127.0.0.1:8000"
                )
            ...
            with st.spinner("🔍 Searching internal documents..."):
                try:
                    ...
                    if response.status_code == 200:
                        ...
                        st.error(
                            "🚫 Access denied. Your role doesn't have permission for this request."
                        )
                    else:
                    ...
                    st.error(
                        "❌ Cannot connect to the backend. Please ensure FastAPI is running."
                    )
                except Exception as e:
